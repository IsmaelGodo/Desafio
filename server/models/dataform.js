...
const getDataformModel = async (user_id) => {
  ...
  const updateDataform = async (form) => {
    const { user_id, sex, age, height, weight, water_gl, activity, cardio_dis, digest_dis, neuro_dis, lung_dis } = form;
    let client, result;
    try {
        client = await pool.connect(); 
        const data = await client.query(dataformQueries.updateDataform,[user_id, sex, age, height, weight, water_gl, activity, cardio_dis, digest_dis, neuro_dis, lung_dis])
        result = data.rowCount
    } catch (err) {
        console.log(err);
        throw err;
...
const getDataformModel = async (id) => {
  const {user_id} = id;
  let client, result;
  try {
      client = await pool.connect(); 
